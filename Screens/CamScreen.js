...
//react native
import { ActivityIndicator, Text, View, ScrollView, StyleSheet, Button, Platform,TouchableOpacity ,Vibration} from 'react-native';
...
export default function CamScreen(props) {
  ...
  const getPrediction = async(tensor) => {
    ...
    if(prediction[0].probability > 0.3) {
      const lst =[ "thunder snake","boa constrictor", "green mamba","worm snake", "Carphophis amoenus", "ringneck snake", "ring-necked snake", "ring snake", "hognose snake", "puff adder", "sand viper", "green snake", "grass snake", "king snake", "kingsnake", "garter snake", "grass snake", "water snake", "vine snake", "night snake", "Hypsiglena torquata", "boa constrictor", "Constrictor constrictor", "rock python", "rock snake", "Python sebae", "Indian cobra", "Naja naja", "green mamba", "sea snake", "horned viper", "cerastes", "sand viper", "horned asp", "Cerastes cornutus", "diamondback", "diamondback rattlesnake", "Crotalus adamanteus", "sidewinder", "horned rattlesnake", "Crotalus cerastes", "nematode", "nematode worm", "roundworm"]
      const classname = JSON.stringify(prediction);
    //   console.log(classname.split(",")[0].split(":")[1])
      if (lst.includes(classname.split(",")[0].split(":")[1].slice(1,classname.length))){
        Vibration.vibrate(10000)
        // console.log("SNAKE!!!")
        setSnakeFound(true)
        setTimeout(() => {setSnakeFound( false)}, 10000)
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={()=>{props.navigation.replace("packscreen")}}>
      ...
      <View style={{flex:1,backgroundColor:"red"}}>
        <View style={{backgroundColor:"black",flex:1}}>
            { !snakeFound&&<Video
                source={require("../assets/bluescan.mp4")}
                rate={1.0}
                volume={1.0}
                isMuted={false}
                resizeMode="cover"
                ...
                ></Video>}
                { snakeFound&&<Video
                    source={require("../assets/redscan.mp4")}
                    rate={1.0}
                    volume={1.0}
                    isMuted={false}
                    resizeMode="cover"
        ...
        <View style={{backgroundColor:"black",flex:.3,justifyContent:"center",alignItems:"center"}}>
            {!snakeFound&&<Text style={{fontFamily:"NunitoExtraBold",fontSize:30,color:"#0066C3"}}>
                NO SNAKE DETECTED
            </Text>}
            {snakeFound&&<Text style={{fontFamily:"NunitoExtraBold",fontSize:30,color:"red"}}>
                SNAKE DETECTED
            </Text>}
        </View> 
